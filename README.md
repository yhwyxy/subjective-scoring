...
进入评分点解析和 Reranker 前，文本评分器先执行两项确定性保护：Unicode NFKC、大小写、连续空白及常见中英文标点规范化后，空白答案直接返回 0 分，精确匹配标准答案直接返回满分；两者置信度均为 1.0，且不会调用 pair scorer。规范化不会删除数字、否定词、SQL 运算符或代码符号。
...
单次请求也可以显式设置不可变控制点，并优先于后端默认曲线；该配置不会影响后续请求：
...
result = service.score({
    ...
    "student_answer": "...",
    "scoring_config": {
        "calibration_points": ((0.0, 0.0), (0.9, 0.5), (1.0, 0.85)),
    },
})
```

控制点必须位于 `[0, 1]`，x 严格递增、y 单调不减。v0.1.6 的远程默认曲线使用上述控制点；在 18 份试卷的 61 道脱敏主观题上，以并发 1、相邻请求至少 1 秒验收，累计绝对误差由 189.1 降至 85.0，净误差为 -10.0，严重高估 0、严重低估 8，最终远程失败 0。
